...
from pipeline import storage, ranker
import asyncio
...
def run_match(
    ...
    try:
        job = loop.run_until_complete(storage.get_job(job_id))
        if not job:
        ...
        if candidate_ids:
            records = loop.run_until_complete(
                asyncio.gather(*[storage.get_candidate(cid) for cid in candidate_ids])
            )
            ...
            candidate_records = loop.run_until_complete(
                storage.get_top_candidates_by_similarity(jd_embedding, shortlist_limit)
            )
